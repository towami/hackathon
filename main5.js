...
loader.load(
  ...
  (object) => {
    ...
    fencingMask.position.set(0, 94, 0); // Adjusted y-position
  ...
  (xhr) => {
    console.log(
      `Fencing Mask loading: ${(xhr.loaded / xhr.total) * 100}% loaded`
    );
  },
  (error) => {
...
const sectionScrollHeight =
  (totalScrollHeight - animationScrollHeight) / (sections.length - 1); // Remaining scroll divided among sections
